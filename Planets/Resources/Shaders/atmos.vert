...
void main()
{
    ...
    if (dot(lookNorm, norm) > 0)
    {
        ...
        if (length(cameraPos - center) > atmosRad)
        {
            color = vec4(calculateLight(relativePos + lookNorm * mat3(DistThroughAtmos[1]), lookNorm, DistThroughAtmos[0], vec3(0, 0, 0)), 1);
        } else {
            color = vec4(calculateLight(relativePos, lookNorm, length(cameraPos - center - aPos), vec3(0, 0, 0)), 1);
        }
